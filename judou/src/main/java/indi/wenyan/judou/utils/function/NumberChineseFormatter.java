...
// edit by gyxx_xc, 2026,
// delete 大写 number convert, convert to traditional Chinese, remove some unused code and inline some small function
...
public class NumberChineseFormatter {
    ...
    public static final String NEGATIVE = "負";
    ...
    public static String format(int amount) {
        ...
        // 负数
        if (amount < 0) {
            chineseStr.append(NEGATIVE);
            amount = -amount;
        }

        chineseStr.append(longToChinese(amount));
    ...
    private static String longToChinese(long amount) {
        ...
        if (partValue > 0) {
            ...
            partChinese = thousandToChinese(partValue);
            chineseStr.insert(0, partChinese + WAN);
        ...
        if (partValue > 0) {
            ...
            partChinese = thousandToChinese(partValue);
            chineseStr.insert(0, partChinese + YI);
        ...
        if (partValue > 0) {
            if (parts[2] == 0) {
                chineseStr.insert(0, YI);
            }
            partChinese = thousandToChinese(partValue);
            chineseStr.insert(0, partChinese + WAN);
        }

        if (!chineseStr.isEmpty() && ZERO.charAt(0) == chineseStr.charAt(0)) {
    ...
    private static String thousandToChinese(int amountPart) {
        ...
        for (int i = 0; temp > 0; i++) {
            ...
            if (digit == 0) { // 取到的数字为 0
                ...
                chineseStr.insert(0, DIGITS[digit] + CHINESE_UNIT_NAME[i]);
                lastIsZero = false;
            }
            temp = temp / 10;
        }
    ...
    private static void addPreZero(@NotNull StringBuilder chineseStr) {
        if (chineseStr.isEmpty()) {
            return;
        }
        if (ZERO.charAt(0) != chineseStr.charAt(0)) {
            chineseStr.insert(0, ZERO);
        }
    }
}
