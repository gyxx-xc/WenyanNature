...
public class WenyanValueData {
    ...
    public void generateCode(Filer filer) throws IOException {
        ...
        generateWenyanType(classBuilder, ClassName.get(packageName, originName + "Object"));
    ...
    private void generateWenyanType(TypeSpec.Builder classBuilder, ClassName name) {
        FieldSpec wenyanType = FieldSpec.builder(ParameterizedTypeName.get(WENYAN_TYPE_CLASS, name), "TYPE")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
                .initializer("new $T<>($S, $T.class)", WENYAN_TYPE_CLASS, name.simpleName().replaceAll("([a-z])([A-Z])", "$1_$2").toLowerCase(), name)
                .build();
        MethodSpec wenyanTypeMethod = MethodSpec.methodBuilder("type")
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(Override.class)
                .addStatement("return TYPE")
                .returns(ParameterizedTypeName.get(WENYAN_TYPE_CLASS, WildcardTypeName.subtypeOf(Object.class)))
                .build();
    ...
    private void generateWenyanObjectType(TypeSpec.Builder classBuilder) {
        ...
        typeClassBuilder.addMethod(MethodSpec.constructorBuilder()
                ...
                .build());
        generateWenyanType(typeClassBuilder, ClassName.get(packageName, originName + "Object", originName + "ObjectType"));
        var createObjectMethodBuilder = MethodSpec.methodBuilder("createObject")
                .addAnnotation(Override.class)
                .addModifiers(Modifier.PUBLIC)
                .returns(I_WENYAN_OBJECT_CLASS)
                .addParameter(ParameterSpec.builder(ParameterizedTypeName.get(ClassName.get(List.class), I_WENYAN_VALUE_CLASS), "argsList").build())
                .addException(WENYAN_THROW_EXCEPTION_CLASS);
