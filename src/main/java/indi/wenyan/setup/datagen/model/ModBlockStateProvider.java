...
public class ModBlockStateProvider extends ModelSubProvider {
    ...
    protected void registerModels() {
        ...
        for (var block : WenyanBlocks.RUNNER_BLOCK.getBlocks()) {
            registerFuluBlock(block, "block/template_hand_runner");
        }
    ...
    private void registerFuluBlock(Block block) {
        registerFuluBlock(block, "block/template_runner_block");
    }

    private void registerFuluBlock(Block block, String templateName) {
        var template = new ModelTemplate(Optional.of(
                Identifier.fromNamespaceAndPath(WenyanProgramming.MODID, templateName)),
                Optional.empty(),
                TextureSlot.TEXTURE);
        ...
        // copy from lever
        var rotations = PropertyDispatch.modify(BlockStateProperties.ATTACH_FACE, BlockStateProperties.HORIZONTAL_FACING)
        ...
        blockModels.blockStateOutput.accept(MultiVariantGenerator.dispatch(block, model).with(rotations));
    }
