package indi.wenyan.client.gui.llm.backend;
...
public class CodeField {
    ...
    private boolean seekCursorNextPlaceholder() {
        indi.wenyan.client.gui.code_editor.backend.behaviour.CodeField.Placeholder next = backend.getPlaceholders().stream()
                .filter(p -> p.index() > backend.getCursor())
                .min(Comparator.comparingInt(indi.wenyan.client.gui.code_editor.backend.behaviour.CodeField.Placeholder::index))
                .orElse(null);
        if (next != null) {
            selecting = false;
